        ...
        with open(keyword + '_applist.csv', 'a') as the_file:
            for a in apps:
                ...
                if(package_name not in package_name_list):                
                    ...
                    json_file.write(json.dumps(details))
                    json_file.write(',')
                    ...
                    the_file.write(package_name+ ',' + title  + ',' + author_without_comma)
                    the_file.write('\n')
        json_file.write(']')
...
for title in title_list:
    ...
    if(title_counter %BATCH_SIZE_TITLE ==0):
        batch += BATCH_SIZE_TITLE
    with open('title/'+keyword +'_'+ str(batch) + '_' + '_applist_secondary.csv', 'a') as the_file: 
        with open('title/'+keyword +'_'+ str(batch) + '_' + '_secondary_jsondump.json', 'a') as json_file:
                ...
                if(package_name not in package_name_list):
                    ...
                    for keyword in keywords:
                        if(keyword in details_description.lower()):
                            ...
                            authors_file.write( package_name+ ',' + title  + ',' + author_without_comma)
                            authors_file.write('\n')
                 
                            title_json_file.write(json.dumps(details))
                            title_json_file.write(',')
                            break
